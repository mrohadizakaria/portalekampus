        ...
        if ($this->ListTransactionRepeater->Items->Count() > 0 ) {
            ...
            if ($datamhs['no_transaksi'] == 'none') {
                ...
                if($this->DB->checkRecordIsExist('nim','transaksi',$nim," AND tahun='$ta' AND idsmt='$idsmt' AND commited=0")) {
                    ...
                    $this->DB->query ('BEGIN');
                    $str = "INSERT INTO transaksi SET no_transaksi=$no_transaksi,no_faktur='$no_faktur',kjur='$ps',tahun='$ta',idsmt='$idsmt',idkelas='$idkelas',no_formulir='$no_formulir',nim='$nim',tanggal=NOW(),jumlah_sks=9,disc=0,userid='$userid',date_added=NOW(),date_modified=NOW()";                                    
                    if ($this->DB->insertRecord($str)) {
                        $str = "INSERT INTO transaksi_detail SET idtransaksi_detail=NULL,no_transaksi=$no_transaksi,idkombi=14,dibayarkan=1200000,jumlah_sks=9";
